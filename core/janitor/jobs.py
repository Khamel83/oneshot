...
def generate_onboarding(project_dir: Optional[str] = None) -> dict:
    ...
    # Do not export into Claude memory. Large auto-generated CLAUDE.local.md
    # files degrade startup context and have caused repo-specific warnings.
    # Keep the full summary in .janitor/onboarding.md for explicit, on-demand use.
    return {
        "status": "ok",
        "summary_length": len(summary),
        "project_type": project_type,
        "claude_local_export": "disabled",
    }
...
def generate_pending_tasks(project_dir: Optional[str] = None) -> dict:
    """Convert signal files into an actionable task list.

    Pure-compute — no LLM call. Reads test-gaps, redo-queue, blockers,
    code-smells, knowledge-risk, and doc-staleness. Produces
    .janitor/pending-tasks.md with prioritized items.

    Deduplicates against previous run to avoid churning the same tasks.
    """
    project_dir = project_dir or os.getcwd()
    ...
    # HIGH: Redo queue (failed/low-score work)
    redo = _read_json("redo-queue.json", project_dir)
    if isinstance(redo, list):
        for item in redo[:5]:
            desc = item.get("description", item.get("task", "redo unknown work"))
            score = item.get("score", "?")
            blockers.append(f"**[high]** Redo: {desc} (scored {score})")
    elif isinstance(redo, dict):
        for item in redo.get("items", [])[:5]:
            desc = item.get("description", "redo unknown work")
            score = item.get("score", "?")
            blockers.append(f"**[high]** Redo: {desc} (scored {score})")

    # HIGH: Unresolved blockers
    bl = _read_json("blockers.json", project_dir)
    if isinstance(bl, dict):
        for b in bl.get("blockers", [])[:5]:
            content = b.get("content", str(b))[:120]
            blockers.append(f"**[blocker]** {content}")

    # HIGH: Test gaps
    tg = _read_json("test-gaps.json", project_dir)
    if isinstance(tg, dict):
        for g in tg.get("gaps", [])[:10]:
            f = g.get("source_file", g.get("file", "?"))
            tasks.append(f"**[high]** Add tests for {f}")

    # MEDIUM: Oversized functions (>200 lines)
    cs = _read_json("code-smells.json", project_dir)
    if isinstance(cs, dict):
        for fn in cs.get("oversized_functions", []):
            if fn.get("lines", 0) >= 200:
                tasks.append(
                    f"**[medium]** Refactor {fn['function']} in {fn['path']} ({fn['lines']} lines)"
                )
        # MEDIUM: Oversized files (>800 lines)
        for fi in cs.get("oversized_files", []):
            if fi.get("lines", 0) >= 800:
                tasks.append(
                    f"**[medium]** Split {fi['path']} ({fi['lines']} lines) into smaller modules"
                )

    # LOW: Knowledge risk (sole contributor)
    kr = _read_json("knowledge-risk.json", project_dir)
    if isinstance(kr, dict):
        for r in kr.get("at_risk", [])[:5]:
    ...
    if isinstance(ds, dict):
        stale = [
            f for f in ds.get("stale_files", [])
            if f.get("days_since_edit", 0) > 60
            and not f.get("file", "").startswith("archive/")
        ][:5]
        for f in stale:
            tasks.append(
                f"**[low]** Update {f['file']} — {f['days_since_edit']} days stale"
            )

    if not blockers and not tasks:
        prev = jdir / "pending-tasks.md"
        if prev.exists():
            prev.unlink()
        return {"status": "no_tasks", "blockers": 0, "tasks": 0}

    # Build output
    parts = []
    if blockers:
        parts.append("## Blockers (need your input)\n")
    ...
    # Dedup: skip if output is identical to previous run
    prev_path = jdir / "pending-tasks-prev.md"
    cur = jdir / "pending-tasks.md"
    if cur.exists() and cur.read_text().strip() == output:
        return {"status": "fresh", "blockers": len(blockers), "tasks": len(tasks)}

    # Rotate: current becomes prev, write new current
    if cur.exists():
        cur.rename(prev_path)
    jdir.mkdir(parents=True, exist_ok=True)
    cur.write_text(output)
...
def review_pending_tasks(project_dir: Optional[str] = None, min_score: int = 3) -> dict:
    """Send draft pending tasks to a free model for quality scoring.

    Returns filtered task list. Drops any task scored below min_score (1-5).
    Blockers are always kept — they require human judgment regardless.
    """
    project_dir = project_dir or os.getcwd()
    jdir = _janitor_dir(project_dir)
    cur = jdir / "pending-tasks.md"
    ...
    if not cur.exists():
        return {"status": "no_tasks", "kept": 0, "dropped": 0}

    draft = cur.read_text().strip()
    if not draft:
        return {"status": "no_tasks", "kept": 0, "dropped": 0}

    lines = draft.split("\n")
    task_lines = [
        {"n": i, "text": l.strip(), "is_task": bool(l.strip())}
        for i, l in enumerate(lines)
        if l.strip() and not l.strip().startswith("#")
    ]

    if len(task_lines) < 2:
        return {"status": "too_few", "kept": len(task_lines), "dropped": 0}

    # Build a numbered list for the reviewer
    numbered = "\n".join(f'{i+1}. {t["text"]}' for i, t in enumerate(task_lines))

    from core.janitor.worker import extract_structured

    try:
        result = extract_structured(
            f"Score each task 1-5 for actionability.\n"
            f"5 = immediately actionable, clear scope\n"
            f"4 = actionable, clear scope\n"
            f"3 = useful but vague or lower priority\n"
            f"2 = marginal, could wait indefinitely\n"
            f"1 = noise, not useful\n\n"
            f"Tasks:\n{numbered}\n\n"
            "Respond with JSON only: {\"scores\": [{\"n\": int, \"score\": int}]}",
            system="Be harsh. If a task is vague or low-impact, score it 1-2.",
            schema_hint='{"scores": [{"n": int, "score": int}]}',
            quality="smart",
        )
    except RuntimeError as e:
        return {"status": "llm_unavailable", "kept": len(task_lines), "dropped": 0, "reason": str(e)}

    if not isinstance(result, dict) or "scores" not in result:
        return {"status": "parse_failed", "kept": len(task_lines), "dropped": 0}

    score_map = {s["n"]: s.get("score", 3) for s in result["scores"]}

    kept_lines = []
    dropped = 0
    in_blockers = False

    # Build set of 0-indexed task_lines to drop (low score, not in blockers section)
    drop_indices = set()
    ...
    for t in task_lines:
        if "## Blockers" in "\n".join(lines[:t["n"] + 1]):
            ...
            in_blockers_section = False
        score = score_map.get(t["n"] + 1, 3)
        if score < min_score and not in_blockers_section:
            drop_indices.add(t["n"])

    task_text_set = {t["text"] for t in task_lines}
    dropped = 0
    kept_lines = []

    for line in lines:
        if line.strip() in task_text_set:
            for t in task_lines:
                if t["text"] == line.strip() and t["n"] in drop_indices:
                    dropped += 1
                    break
            else:
    ...
    if output:
        ...
        for i, line in enumerate(out_lines):
            ...
            if m:
                counter += 1
                out_lines[i] = f"{counter}. {line[m.end():]}"
        output = "\n".join(out_lines)

    if not output.strip():
        cur.unlink()
        return {"status": "all_dropped", "kept": 0, "dropped": dropped}

    cur.write_text(output)

    return {
        "status": "reviewed",
        "kept": len([l for l in kept_lines if l.strip() and not l.startswith("#")]),
        "dropped": dropped,
        "scores_applied": len(score_map),
    }
...
def run_session_start(project_dir: Optional[str] = None) -> str:
    ...
    # Generate pending tasks from signals (pure-compute)
    try:
        pt = generate_pending_tasks(project_dir)
        if pt.get("status") == "ran":
            b, t = pt.get("blockers", 0), pt.get("tasks", 0)
            alert = []
            if b > 0:
                alert.append(f"{b} blockers")
            if t > 0:
                alert.append(f"{t} tasks")
            if alert:
                parts.append(f"pending tasks: {', '.join(alert)}")
    except Exception:
        pass
...
def run_session_end(project_dir: Optional[str] = None) -> None:
    ...
    # Generate pending tasks (pure-compute, no API key needed)
    try:
        pt = generate_pending_tasks(project_dir)
        # Quality-review with cheap free model (only if tasks were generated)
        if pt.get("status") == "ran" and pt.get("tasks", 0) > 0:
            review_pending_tasks(project_dir)
    except Exception:
        pass
