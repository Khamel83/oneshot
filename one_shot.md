...
**Security**: SOPS-encrypted secrets (never commit plaintext, always in git)  
...
Secrets:      SOPS + age (encrypt secrets, commit safely to git)
```
...
- **Central secrets repo (SOPS-encrypted)**: ONE place for ALL secrets
  - Private GitHub repo with ALL your secrets (OpenRouter, GitHub token, etc.)
  - SOPS-encrypted, safe to push to GitHub
  - Every project pulls from central repo (no duplication)
  - One age key unlocks everything
  - Update secrets once, all projects get it
  - $0 cost, works anywhere you're logged into GitHub
  - See CENTRAL_SECRETS.md for setup
...
- **Never Commit Plaintext Secrets**
  - ALL secrets MUST be SOPS-encrypted before git commit
  - Commit `.env.encrypted`, never `.env`
  - One age key in 1Password, all projects use the same key
  - Safe to push to public OR private repos (encrypted either way)
...
## Q14. Central Secrets Setup - REQUIRED

**Every ONE_SHOT project pulls secrets from your central secrets repo.**

### Part A: Do you have an age key?
...
**If NO** - Generate one NOW (takes 10 seconds):
```bash
mkdir -p ~/.config/sops/age
age-keygen -o ~/.config/sops/age/keys.txt
```

**CRITICAL**: Back up to 1Password NOW (you can't decrypt without it)
...
### Part B: Do you have a central secrets repo?

**If NO** - Create it NOW (takes 2 minutes):
...
EOF
# Replace YOUR_AGE_PUBLIC_KEY_HERE with output from above
...
# Add any other secrets you use across projects
EOF

# 4. Encrypt
sops --encrypt secrets.env > secrets.env.encrypted
rm secrets.env  # Delete plaintext
...
git init
git add .
git commit -m "Central secrets"
gh repo create secrets-vault --private --source=. --push
```

**Your central secrets repo URL**:
```
...
- Update secrets once, all projects get it
- Never commit plaintext secrets (only .encrypted files)
- See CENTRAL_SECRETS.md for details

**Provide the repo URL**:
```
[PASTE YOUR SECRETS REPO URL HERE]
```
...
Provide values later in `.env`, encrypted with SOPS, not here in plaintext.

**Secrets Management Pattern** (Default):
1. Create `.env` with secrets
2. Encrypt to `.env.encrypted` using SOPS (see SOPS_INTEGRATION.md)
3. Commit `.env.encrypted` to git (safe to push)
4. Add `.env` to `.gitignore` (never commit plaintext)
5. Auto-decrypt on setup with `scripts/setup_secrets.sh`

**Why SOPS?**
- Free & open-source ($0 cost)
- Works offline (no cloud service)
- Encrypted files safe in git
- Auto-decrypt when authorized
- No keys to remember (uses age/GPG)
...
echo "[*] SOPS (secrets encryption - REQUIRED):"
if command -v sops >/dev/null 2>&1; then
  ...
  if [ -f ~/.config/sops/age/keys.txt ]; then
    ...
    echo "Generate with: mkdir -p ~/.config/sops/age && age-keygen -o ~/.config/sops/age/keys.txt"
  fi
else
  echo "❌ SOPS not installed"
  echo "Install with: brew install sops age  (Mac)"
fi
...
- **Setup central secrets integration (MANDATORY)**:
  - Create `scripts/setup_secrets.sh` configured with user's secrets repo URL (from Q14)
  - Create `scripts/update_secrets.sh` for editing central secrets
  - Update `.gitignore` to ignore `.env` (plaintext)
  - No `.env.example` or `.env.encrypted` needed (everything comes from central repo)
  - See CENTRAL_SECRETS.md if user doesn't have secrets repo yet
...
- **v1.6** (2024-11-29)
  - **BREAKING CHANGE**: Central Secrets now MANDATORY (no more per-project secrets)
  - **Philosophy**: ONE place for ALL secrets (github.com/you/secrets-vault)
    - OpenRouter key once, not per-project
    - Update once, all projects get it
    - No duplication, no confusion
  - **Added**: Q14 (Central Secrets Setup) to Core Questions
    - Asks for age key (one-time)
    - Asks for secrets-vault repo URL
    - Auto-configures all projects to pull from central repo
  - **Added**: `CENTRAL_SECRETS.md` - simple 5-minute setup guide
  - **Simplified scripts**:
    - `scripts/setup_secrets.sh` - pulls from central repo (no per-project encryption)
    - `scripts/update_secrets.sh` - edits central repo secrets
    - Removed: `encrypt_secrets.sh`, `edit_secrets.sh` (no longer needed)
  - **Updated**: Phase 0 - projects reference central secrets repo URL
  - **Updated**: Canonical FOSS stack - "Central secrets repo (SOPS-encrypted)"
  - **Updated**: Archon Principles - "Never Commit Plaintext Secrets" now refers to central pattern
  - **Result**: Simplest possible workflow - one key, one repo, all projects work
- **v1.5** (2024-11-26)
